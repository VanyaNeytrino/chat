class Chat extends React.Component {
  ...
  componentDidMount () {
    ...
    const user = localStorage.getItem('user') || `${prompt("Введите ваше имя").replace(/\:|\@/g, "")}@${randomColor({luminosity: 'dark'})}@${random(1000, 2000)}`
    ...
    server.onopen = () => {
      const payload = 
        JSON.stringify({username: user, message: "присоеденился к комнате."})
    ...
    server.onclose = () => {
      const payload = 
        JSON.stringify({username: user, message: "покинул комнату."})
  ...
  render () {
    ...
    const $chat = 
      <div>
        <ul>{messages}</ul>
        <input autofocus='true' placeholder='Напишите ваше сообщение'
